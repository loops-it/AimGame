...
export default function TableProvider({ data, loading, emptyMessage, children, pagination, className, itemsPerPage, currentPage, setCurrentPage }) {
    ...
    return (
        <div className="overflow-x-auto relative">
            ...
            {pagination && itemsPerPage && data?.length > 0 && (data?.length / itemsPerPage) > 1 &&
                <div className='flex items-center gap-2 justify-end mt-5 pr-4 pb-5' >
                    ...
                    <div>{currentPage}</div>
                    <div>OF</div>
                    <div>{data?.length / itemsPerPage}</div>
                    <button
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" className={`w-5 h-5`} width="13.503" height="23.616" viewBox="0 0 13.503 23.616">
                            <path d="M20.679,18,11.742,9.07a1.681,1.681,0,0,1,0-2.384,1.7,1.7,0,0,1,2.391,0L24.258,16.8a1.685,1.685,0,0,1,.049,2.327L14.14,29.32a1.688,1.688,0,0,1-2.391-2.384Z" transform="translate(-11.246 -6.196)" fill={`${currentPage == (data?.length / itemsPerPage) ? "#6D6A6A" : "#30385E"}`} />
                        </svg>
                    </button>
                </div>
            }
